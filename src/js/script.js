...
function gameOver() {
  new Audio("/flappy-bird/src/sounds/sounds_gameover.wav").play();
  gameStopped = true;
  showGameOverScreen();
  stopBlockAnimation();
  stopGravity();
...
function characterJump() {
  ...
  const jumpInterval = setInterval((_) => {
    ...
    if (jumpCount > 20) {
      new Audio("/flappy-bird/src/sounds/sounds_fly.mp3").play();
...
function handleStarDetection() {
  ...
  if (detectColision(character, star)) {
    new Audio("/flappy-bird/src/sounds/sounds_star.wav").play();
    scoreTotal += 150;
    hideStar();
    changeScoreUi();
  }
...
function handleCharacterCollision() {
  ...
  if (colisionBlock && !colisionHole) {
    ...
    soundCount++;
    if (soundCount > 35) {
      new Audio("/flappy-bird/src/sounds/sounds_hole.wav").play();
      soundCount = 0;
    }
    changeScoreUi();
    if (gameStopped) return;
